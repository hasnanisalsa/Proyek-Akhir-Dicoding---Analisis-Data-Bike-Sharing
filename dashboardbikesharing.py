...
# Fungsi kondisi cuaca
def total_weathersit_df(day_df): 
    cuaca_df = day_df.groupby(by="weathersit").cnt.sum().reset_index() 
    return cuaca_df
...
cuaca_df = total_weathersit_df(main_df_day)
...
# 4
# Menampilkan dampak kondisi cuaca terhadap penyewaan sepeda
st.subheader("Dampak Kondisi Cuaca terhadap Penyewaan Sepeda")

colors1 = ["#90CAF9", "#D3D3D3", "#D3D3D3"]
fig, ax = plt.subplots(figsize=(10,6))
sns.barplot(
        y="cnt", 
        x="weathersit",
        data=cuaca_df.sort_values(by="weathersit"),
        palette=colors1,
    )
ax.set_ylabel(None)
ax.set_xlabel("Kondisi Cuaca (1: Clear and Cloudy, 2: Misty, 3: Light Snow/Rain)")
ax.tick_params(axis='x', labelsize=12)
ax.tick_params(axis='y', labelsize=12)
...
# 6
# Menampilkan jumlah penyewaan sepeda
